...
/// Detect the environment name from a .env filename.
...
/// Parse the raw text of a .env file into structured variables.
fn parse_env_content(content: &str) -> Vec<EnvVariable> {
    ...
    while i < lines.len() {
        ...
        if let Some(eq_pos) = effective.find('=') {
            ...
            // Detect quoted values and handle multiline
            if val_part.starts_with('"') {
                has_quotes = true;
                // Possibly multiline — find the closing quote
            ...
            // Line without '=' that is not a comment — treat as raw
            variables.push(EnvVariable {
                key: String::new(),
                value: String::new(),
                line: line_number,
...
/// Derive a 32-byte key from a password using SHA-256 (simple KDF for local use).
fn derive_key(password: &str) -> [u8; 32] {
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};

    // Simple key stretching: hash the password multiple times to fill 32 bytes.
    // For production you would use argon2 or scrypt, but for local desktop
    // file encryption this provides a reasonable baseline.
    let mut key = [0u8; 32];
...
#[tauri::command]
fn scan_env_files(path: String) -> Result<Vec<EnvFileInfo>, String> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        // Match .env* patterns but exclude .env.encrypted
...
#[tauri::command]
fn read_env_file(path: String) -> Result<Vec<EnvVariable>, String> {
    let content = fs::read_to_string(&path).map_err(|e| format!("Failed to read {}: {}", path, e))?;
    Ok(parse_env_content(&content))
}

#[tauri::command]
fn write_env_file(path: String, variables: Vec<EnvVariable>) -> Result<(), String> {
    let mut lines: Vec<String> = Vec::new();

    for var in &variables {
        if var.is_empty && var.key.is_empty() {
            // Preserve blank lines and raw lines
...
#[tauri::command]
fn diff_env_files(left: String, right: String) -> Result<Vec<DiffResult>, String> {
    let left_content =
    ...
    // Build maps of key -> value (skip comments/empties)
    let _left_map: HashMap<String, String> = left_vars
        .iter()
    ...
    // Iterate left keys preserving order
    for var in &left_vars {
        ...
        match right_map.get(&var.key) {
            Some(right_val) => {
                let status = if *right_val == var.value {
                    "same"
                } else {
                    "changed"
                };
...
#[tauri::command]
fn validate_env(variables: Vec<EnvVariable>) -> Vec<ValidationIssue> {
    ...
    for var in &variables {
        ...
        if var.value != var.value.trim() {
            issues.push(ValidationIssue {
                ...
                message: format!(
                    "Key '{}' has leading or trailing whitespace in its value",
                    var.key
                ),
                severity: "warning".into(),
            });
        ...
        // URL validation for keys that look like URLs
        ...
        if key_lower.contains("url")
            ...
            || key_lower.contains("origin")
        {
            if !var.value.starts_with("http://")
                && !var.value.starts_with("https://")
                && !var.value.starts_with("ws://")
                && !var.value.starts_with("wss://")
                && !var.value.starts_with("mongodb://")
                && !var.value.starts_with("mongodb+srv://")
                && !var.value.starts_with("redis://")
                && !var.value.starts_with("postgres://")
                && !var.value.starts_with("mysql://")
                && !var.value.starts_with("amqp://")
                && !var.value.contains("localhost")
                && !var.value.contains("127.0.0.1")
            {
                issues.push(ValidationIssue {
                    key: var.key.clone(),
                    value: var.value.clone(),
        ...
        // Port validation for keys that look like ports
            ...
            if let Ok(port) = var.value.parse::<u32>() {
                if port == 0 || port > 65535 {
                    issues.push(ValidationIssue {
                        ...
                        message: format!(
                            "Key '{}' has port value {} outside valid range 1-65535",
                            var.key, port
                        ),
                        severity: "error".into(),
                    });
    ...
    // Deduplicate duplicate-key warnings (only report once per key)
...
#[tauri::command]
fn encrypt_env(path: String, password: String) -> Result<String, String> {
    let content =
...
#[tauri::command]
fn decrypt_env(path: String, password: String) -> Result<Vec<EnvVariable>, String> {
    let encoded =
        fs::read_to_string(&path).map_err(|e| format!("Failed to read {}: {}", path, e))?;
...
#[tauri::command]
fn generate_example(variables: Vec<EnvVariable>) -> String {
    let mut lines: Vec<String> = Vec::new();
...
#[tauri::command]
fn check_gitignore(project_path: String) -> Result<Vec<GitIgnoreStatus>, String> {
    ...
    let results: Vec<GitIgnoreStatus> = env_files
        ...
        .map(|file_path| {
            ...
            let ignored = gitignore_patterns.iter().any(|pattern| {
                ...
                // Glob-style: .env* matches .env, .env.local, etc.
