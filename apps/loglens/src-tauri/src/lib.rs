...
#[tauri::command]
fn read_log_file(path: String, max_lines: Option<usize>) -> Result<Vec<String>, String> {
...
#[tauri::command]
fn tail_log_file(app: AppHandle, path: String) -> Result<(), String> {
...
/// Stop tailing a file.
#[tauri::command]
fn stop_tailing(app: AppHandle, path: String) -> Result<(), String> {
    let state = app.state::<Arc<Mutex<WatcherState>>>();
    let mut state = state.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
fn list_log_files(dir: String) -> Result<Vec<LogFileInfo>, String> {
...
fn read_new_lines(path: &str, position: &Arc<Mutex<u64>>) -> Result<Vec<String>, String> {
